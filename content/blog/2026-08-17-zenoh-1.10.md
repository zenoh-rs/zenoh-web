---
title: "Zenoh 1.10.x: Mucalinda"
date: 2026-08-18
menu: "blog"
weight: 20260817
description: "18th August, 2026 -- Paris."
draft: false 
---

Zenoh 1.10.x **Mucalinda** has landed!

**Mucalinda** is the guardian Nāga who, as the story goes, rose during a great storm to coil around the meditating Buddha and shield him beneath his hood. A fitting name for a chapter shaped by changing tides, and a quiet reminder that what truly matters can always find guardians willing to stand watch over it.

This release concentrates on the paths data actually travels: how fast bytes come off the wire, how much you can see about where the time goes, and how little ceremony it takes to reach Zenoh from your language of choice. On Linux, an optional `io_uring` receive backend shortens round-trip latency on eligible unicast links. A new timestamp stack lets a message record where it has been, turning end-to-end latency from a single number into a per-hop breakdown. And the JVM bindings were rebuilt on generated JNI code, more than doubling throughput on small samples.

Around those, shared-memory setup shed a class of boilerplate, the advanced subscriber stopped accumulating per-publisher state without bound, and Zenoh-Python became installable with `apt`.

Key highlights of this release include:

* **Zenoh-Java and Zenoh-Kotlin rework**: both bindings rebuilt on a common generated JNI layer, more than doubling throughput on small samples.
* **io_uring receive path**: an optional Linux backend cutting round-trip latency by roughly 20% on eligible unicast links.
* **Timestamp instrumentation**: messages can carry a stack of timestamps recorded at send, at each routing hop, and at delivery.
* **Reliable SHM handoff**: stronger end-to-end ownership model for shared-memory transport.
* **Waitable transport SHM provider**: wait for the transport-managed shared-memory provider instead of polling it, consistently from Rust, C and C++.
* **Selective SHM transport optimization**: choose which message categories are automatically promoted to shared memory.
* **Advanced subscriber retention period**: per-publisher recovery state is now garbage collected instead of living for the lifetime of the subscriber.
* **Zenoh-Python Debian package**: `apt install python3-eclipse-zenoh`, no virtual environment required.

## Breaking changes

1. The `source_info` setter was removed from `AdvancedSubscriber`—setting it there was a no-op. See [Source Info update](#source-info-update).
2. In Zenoh-Java and Zenoh-Kotlin, `Timestamp` is now a Zenoh type and `SampleMiss` was renamed to `Miss`. See [Zenoh-Java and Zenoh-Kotlin](#zenoh-java-and-zenoh-kotlin).
3. Behavioral change: an `AdvancedSubscriber` with recovery enabled now discards the state of publishers that have been silent for longer than `retention_period` (one hour by default), so losses that occurred during that silence are no longer detected. See [Retention period for the AdvancedSubscriber's RecoveryConfig](#retention-period-for-the-advancedsubscribers-recoveryconfig).
4. Reliable SHM handoff — [#2646](https://github.com/eclipse-zenoh/zenoh/pull/2646) - old zenoh builds will never negotiate to use SHM with ones after this release

## Zenoh

### Reliable SHM handoff

* Reliable SHM handoff — [#2646](https://github.com/eclipse-zenoh/zenoh/pull/2646) by @yellowhatter

Zenoh's shared-memory transport can pass payloads between local processes without copying their contents through the network stack. This makes SHM extremely efficient, but it also introduces an important lifetime requirement: the memory backing an SHM message must remain valid until the receiving side has actually obtained its own reference to it.

PR #2646 strengthens this lifetime guarantee by introducing an explicit **SHM handoff mechanism** for reliable unicast transports. When an SHM-backed message is accepted into the transmission pipeline, Zenoh now keeps a hard reference to its shared-memory buffer on the sending side. That reference is not released merely because the message has been queued for transmission; it is kept until the receiver confirms that it has reached the message and successfully mounted the SHM buffer.

This distinction matters whenever transport processing is delayed. A reliable message may spend a significant amount of time inside Zenoh's queues because of congestion, backpressure, scheduling delays, or a slow receiver. The SHM object's application-visible lifetime may meanwhile have ended on the publisher side. With handoff tracking, transport ownership bridges this gap: once Zenoh accepts an SHM buffer for reliable delivery, the sender keeps the underlying memory alive until ownership has effectively crossed to the receiver.

The synchronization itself is intentionally lightweight. During transport establishment, the two peers exchange shared-memory metadata together with identifiers for **handoff counters**. For reliable links, counters are allocated per Zenoh priority. Before an SHM reference is committed to the transport pipeline, the sender increments the corresponding counter and retains a `ShmBufHardRef`. When the receiver converts the received SHM descriptor into a mounted buffer, it decrements that counter. The sender can then release exactly the references that have been acknowledged by the receiver.

Keeping the counters per priority is important because Zenoh's reliable transport preserves ordering independently across its priority channels. This allows handoff acknowledgement to remain extremely compact: Zenoh does not need to attach a separate acknowledgement message or identifier to every SHM buffer. Instead, the monotonically changing shared counters describe how far the receiver has progressed through each ordered stream, while the sender maintains the corresponding queue of retained references.

The handoff is also integrated transactionally with transport scheduling. SHM references are first staged while the network message is being prepared. They are committed only if the message is successfully pushed into the transport pipeline; if scheduling fails, the temporary counter increments and retained references are cancelled. This keeps the SHM lifetime accounting aligned with what the transport has actually accepted for transmission.

On the receiving side, acknowledgement happens only when the SHM descriptor is processed and converted into an actual shared-memory buffer. Zenoh first mounts the descriptor through the SHM reader and then advances the corresponding handoff counter. Consequently, a message merely being present somewhere in the transport pipeline is not enough to release the producer-side protection—the handoff completes when the receiving process has obtained the buffer.

All of this remains internal to the transport layer. Applications continue publishing and receiving SHM-backed `ZBytes` exactly as before; there is no new handoff API that users need to manage. The change instead makes the existing zero-copy path more robust under real-world conditions where reliable messages may temporarily accumulate inside transport queues.

The new mechanism is deliberately used for **reliable traffic**. Best-effort delivery can drop messages, so a simple ordered-counter handoff cannot safely associate every transmitted reference with a corresponding receive event. Reliable channels provide the ordering and delivery semantics required by this compact acknowledgement scheme, and the implementation therefore enables per-priority handoff tracking for reliable links while leaving it disabled for best-effort links.

PR #2646 also extends Zenoh's SHM establishment protocol to exchange the metadata required by the handoff mechanism and updates the SHM protocol version accordingly. The result is a stronger end-to-end ownership model for shared-memory transport: **once a reliable SHM message enters Zenoh's transport, its backing memory remains protected until the receiver has taken over the reference**, even if that message spends an unusually long time waiting inside the transport pipeline.
...
An independent evaluation compared the same build with and without the `uring` feature on TCP loopback workloads, measuring round-trip time across message sizes:

| Message size | RTT reduction |
| ------------ | ------------- |
| 64 B         | ~24% (within measurement noise) |
| 1 KiB        | ~22% |
| 64 KiB       | ~18% |
| 1 MiB        | ~14% (within measurement noise) |

Throughput remained broadly comparable between the two backends: with `io_uring` enabled, message rates were effectively unchanged. The principal benefit of the current implementation is therefore **lower receive-path latency and fewer data-movement costs**, rather than a dramatic increase in maximum throughput.
...
Timestamp instrumentation is part of Zenoh's **unstable** API and requires the `unstable` feature. It is carried as a protocol extension, so only nodes running 1.10 or later contribute records to the stack.

### Source Info update

The `AdvancedSubscriber` uses the `SourceInfo` (entity ID and sample sequence number) for its own tracking of missing samples. Setting the `SourceInfo` field there is in reality a no-op, so the `source_info` setter was removed from `AdvancedSubscriber`, and the documentation for `SourceInfo` was clarified accordingly.
...
`RecoveryConfig` now exposes a `retention_period` (one hour by default): the state of a publisher from which nothing has been received for that long is garbage collected, along with its periodic query task. Publishers still reported as alive by the liveliness subscriber are never reclaimed. Note that once a source's state has been dropped, losses that happened while it was silent can no longer be detected: the subscriber simply resynchronizes on the next sample it receives from that publisher.

### Selecting the message types optimized by shared memory

Zenoh can transparently offload large message payloads to shared memory (SHM) on local links, avoiding costly copies for high-throughput pub/sub. Previously this "transport optimization" was all-or-nothing.

This release adds a new `transport/shared_memory/transport_optimization/messages` setting that lets you pick exactly which message categories benefit from it. Any combination of `put`, `query`, and `reply` is supported, and the default is all three—so this setting is about opting *out* of automatic promotion for categories where it does not pay off.
...
## Zenoh-C and Zenoh-Cpp

Beyond the blocking SHM provider API described above, Zenoh-Cpp gained two portability improvements.

The library can now be built **without C++ exceptions**, for toolchains and projects that compile with `-fno-exceptions`. This matters for embedded and safety-critical codebases where exceptions are disallowed by policy or unavailable altogether; the error-reporting paths fall back to the same result-based reporting the rest of the API already offers.

Zenoh-Cpp also builds cleanly with **MSVC 2022 in C++20 mode**, which previously failed to compile.
...
In addition, the accept-replies API was stabilized, and the build now uses the [Gradle wrapper](https://docs.gradle.org/current/userguide/gradle_wrapper.html).

## Zenoh-TS

The Nuxt example shipped with Zenoh-TS was extended to exercise a much larger part of the API, making it a more useful starting point for web applications than a minimal pub/sub demo.
...
The new behavior is controlled by four configuration options:
...
A new build-time configuration variable, `Z_RUNTIME_IDLE_READ_TASK_SLEEP` (default `0`), puts the Zenoh-Pico background read task to sleep for the given number of milliseconds when the last read returned nothing, instead of staying awake and polling sockets continuously.

This is mostly of interest in single-threaded mode, where the transport RX buffers are populated manually. There, `zp_spin_once` now reports whether Zenoh-Pico still has immediate work to do—accepting a pending connection, sending a keep-alive, or attempting a socket read—or whether spinning the executor again is pointless until the RX buffer is refilled.
...
Zenoh 1.10.x **Mucalinda** is a release about the parts of the system you normally do not see. Nothing here changes how you write a publisher or a subscriber, but the bytes now leave the kernel faster on Linux, the JVM bindings stop paying a JNI tax on every field, and when something is slower than you expect, the message itself can tell you which hop cost you the time.

Taken together with the smaller changes—waiting instead of polling for the transport SHM provider, bounding advanced-subscriber state, choosing what shared memory is used for, and installing Zenoh-Python straight from `apt`—this is a release that should mostly make itself felt by getting out of your way.
